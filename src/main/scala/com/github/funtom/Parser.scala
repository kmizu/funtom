...
import com.github.funtom.Ast._
import com.github.funtom.Type._
import com.github.funtom.macro_peg.Parser.Fragment
...
class Parser extends Processor[String, Program, InteractiveSession] {
  object Core extends SCombinator with macro_peg.Parser.Fragment {
    object Funtom {
      ...
      lazy val phantomComponent: Parser[String] = """\?[A-Za-z_][a-zA-Z0-9_]*""".r
      ...
      lazy val phantom: Parser[PhantomId] = (%% ~ phantomComponent.filter { n =>
        !KEYWORDS(n)
      } ^^ { case location ~ name => PhantomId(location, name.substring(1)) }) << SPACING_WITHOUT_LF
