...
import com.github.funtom._
import com.github.funtom.Type._
import com.github.funtom.TypedAst.{FunctionLiteral, TypedNode, ValueNode}
import com.github.funtom.runtime.{AssertionError, NotImplementedError, PhantomCallError}
...
class Interpreter extends Processor[TypedAst.Program, Value, InteractiveSession] {interpreter =>
  ...
  object BuiltinEnvironment extends RuntimeEnvironment(None) {
    ...
    define("map") { case List(ObjectValue(list: java.util.List[_])) =>
      NativeFunctionValue{
        case List(fun: FunctionValue) =>
          ...
          var i = 0
          while(i < list.size()) {
            val param: Value = Value.toFuntom(list.get(i).asInstanceOf[AnyRef])
            val result: Value = performFunctionInternal(fun.value, List(ValueNode(param)), env)
            newList.add(Value.fromFuntom(result))
            i += 1
          }
          ObjectValue(newList)
      }
    ...
    define("random") { case List(from: Value, until: Value) =>
      val fromV = from.asInstanceOf[BoxedInt].value
      val untilV = from.asInstanceOf[BoxedInt].value
      BoxedInt((math.random() * untilV).toInt - fromV)
    }

    define("head") { case List(ObjectValue(list: java.util.List[_])) =>
      Value.toFuntom(list.get(0).asInstanceOf[AnyRef])
    }
    define("tail") { case List(ObjectValue(list: java.util.List[_])) =>
      Value.toFuntom(list.subList(1, list.size()))
    }
    define("cons") { case List(value: Value) =>
      NativeFunctionValue{ case List(ObjectValue(list: java.util.List[_])) =>
        val newList = new java.util.ArrayList[Any]
        var i = 0
        newList.add(Value.fromFuntom(value))
        while(i < list.size()) {
          newList.add(list.get(i))
          i += 1
        }
        Value.toFuntom(newList)
      }
    }
    ...
    define("foldLeft") { case List(ObjectValue(list: java.util.List[_])) =>
      NativeFunctionValue{ case List(init: Value) =>
        NativeFunctionValue { case List(fun: FunctionValue) =>
          ...
          while(i < list.size()) {
            val params: List[TypedNode] = List(ValueNode(result), ValueNode(Value.toFuntom(list.get(i).asInstanceOf[AnyRef])))
    ...
    enter(LIST) {
      ...
      define("remove") { case List(ObjectValue(self: java.util.List[_])) =>
        NativeFunctionValue{ case List(a: Value) =>
          ...
          newList.remove(Value.fromFuntom(a))
          ObjectValue(newList)
        }
      }
      define("size") { case List(ObjectValue(list: java.util.List[_])) =>
    ...
    enter(SET) {
      ...
      define("remove") { case List(ObjectValue(self: java.util.Set[_])) =>
        NativeFunctionValue{ case List(a: Value) =>
          ...
          newSet.remove(Value.fromFuntom(a))
          ObjectValue(newSet)
        }
      }
      define("contains") { case List(ObjectValue(self: java.util.Set[_])) =>
        NativeFunctionValue { case List(a: Value) =>
          BoxedBoolean(self.contains(Value.fromFuntom(a)))
        }
      }
  ...
  private def performPhantom(node: TypedAst.PhantomCall, env: RuntimeEnvironment): Value = node match {
    case TypedAst.PhantomCall(type_, location, function, params) =>
      val values = params.map{ p => evaluate(p, env)}
      throw PhantomCallError(function, values)
  }
    ...
    def evalRecursive(node: TypedNode): Value = {
      node match{
        ...
        case TypedAst.ListLiteral(type_, location, elements) =>
          val params = elements.map{e => Value.fromFuntom(evalRecursive(e))}
          val newList = new java.util.ArrayList[Any]
        ...
        case TypedAst.MapLiteral(type_, location, elements) =>
          val params = elements.map{ case (k, v) =>
            (Value.fromFuntom(evalRecursive(k)), Value.fromFuntom(evalRecursive(v)))
          }
          val newMap = new java.util.HashMap[Any, Any]
            ...
            case ObjectValue(value) =>
              ...
              findMethod(value, name, paramsArray) match {
                case UnboxedVersionMethodFound(method) =>
                  val actualParams = paramsArray.map{Value.fromFuntom}
                  Value.toFuntom(method.invoke(value, actualParams:_*))
        ...
        case TypedAst.ObjectNew(type_, location, className, params) =>
          ...
          findConstructor(Class.forName(className), paramsArray) match {
            case UnboxedVersionConstructorFound(constructor) =>
              val actualParams = paramsArray.map{Value.fromFuntom}
              Value.toFuntom(constructor.newInstance(actualParams:_*).asInstanceOf[AnyRef])
        ...
        case call@TypedAst.PhantomCall(type_, location, phantomId, params) =>
          performPhantom(call, env)
