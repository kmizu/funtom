...
import com.github.funtom.Ast.{FunctionCall, ListLiteral, MapLiteral, ObjectNew, _}
import com.github.funtom.Type.{TBoolean, TDynamic}
...
class SyntaxRewriter extends Processor[Ast.Program, Ast.Program, InteractiveSession] {
  ...
  def doRewrite(node: Ast.Node): Ast.Node = node match {
    ...
    case PhantomCall(location, func, params) => PhantomCall(location, doRewrite(func).asInstanceOf[PhantomId], params.map{doRewrite})
