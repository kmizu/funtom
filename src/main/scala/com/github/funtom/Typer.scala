...
import com.github.funtom.TypedAst.TypedNode
import com.github.funtom.Ast.Node
import com.github.funtom.Ast.RecordDeclaration
import com.github.funtom.Type.{TConstructor, _}
...
class Typer extends Processor[Ast.Program, TypedAst.Program, InteractiveSession] {
  ...
  val BuiltinEnvironment: Environment = {
    Map(
      ...
      "random"       -> TScheme(Nil, TFunction(List(TInt, TInt), TInt)),
  ...
  def doType(e: Ast.Node, env: TypeEnvironment, t: Type, s0: Substitution): (TypedNode, Substitution) = {
    ...
    e match {
      ...
      case Ast.PhantomCall(location, phantomId, ps) =>
        val t2 = ps.map{_ => newTypeVariable()}
        //val (typedTarget, s1) = doType(e1, env, TFunction(t2, t), s0)
        val bottom = newTypeVariable()
        val s1 = unify(bottom, t, s0)
        val (tparams, s) = (ps zip t2).foldLeft((Nil:List[TypedNode], s1)){ case ((tparams, s), (e, t)) =>
          val (tparam, sx) = doType(e, env, t, s)
          (tparam::tparams, sx)
        }
        (TypedAst.PhantomCall(s.replace(t), location, TypedAst.PhantomId(bottom, location, phantomId.name), tparams.reverse), s)
