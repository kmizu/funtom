# Funtom: An Experimental Programming Language which has the Language Feature *Phantom Call*

[![Build Status](https://travis-ci.org/kmizu/funtom.png?branch=master)](https://travis-ci.org/kmizu/funtom)

Funtom is a functional programming language based on [Klassic](https://github.com/klassic/klassic).

It has the language feature *Phantom call*.  Phantom call means the function has name and is not implemented yet.

For example, the following Funtom programs throw PhatonCallError:
...
Phantom call is used when the function name is determined and implementation is missing.  Phantom call has type `'a` and can be used in everywhere.

## Instlattion and Quick Start

It requires Java 8 or later.  You can build from sources:

```
$ sbt assembly
$ ls target/scala-2.13
./  ../  classes/  funtom.jar  test-classes/  update/
```

You can execute funtom.jar:

```
$ java -jar funtom.jar

Usage: java -jar funtom.jar (-f <fileName> | -e <expression>)
<fileName>   : read a program from <fileName> and execute it
-e <expression> : evaluate <expression>
```

Write the folowing lines and save it to `hello.fun`

```scala
println("Hello, World!")
```

And run the interpreter by `java -jar funtom.jar hello.nm`:

```
$ java -jar funtom.jar hello.fun
...
separator characters have also line feeds and spaces in Namelss unlike other programming languages.
...
separator characters also include line feeds and spaces in Namelss unlike other programmign languages:
...
Namelss supports various literal.  The followings are explanations:
...
Namelss provides two kinds of comment
...
Namelss is a statically-typed programming language.  A characteristic of type
system of Namelss is `restricted` subtyping.  `restricted` means that implicit
upcast is not allowed and it must be specified explicitly if you need it.

### Hindley-Milner Type Inference

Namelss's type inference is based on HM.  It means that type annotations
...
Namelss has simple object system based on row polymorphism.  For example,
...
Namelss supports some kind of built-in functions.
...
- `ToDo: () => Unit`  
    Throws `funtom.runtime.NotImplementedError` when evaluated.
    ```
...
- `assert: (condition:Boolean) => Unit`  
    Asserts that the `condtion` should be true, and throws `funtom.runtime.AssertionError` if the `condition` is false.
  ```
  assert(2 == 1 + 1)  // => OK
...
- `assertResult: (expected:Any)(actual:Any) => Unit`  
    Asserts that the `actual` value should be equal to the `expected` value, and throws `funtom.runtime.AssertionError` if the `actual` value is not equal to the `expected` value.
    ```
    val add = (x, y) => {
      x + y
    }
